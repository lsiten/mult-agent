...
# from gateway.middleware import auth_middleware  # Electron: 完全禁用认证
auth_middleware = None  # Electron: 强制禁用
...
MAX_REQUEST_BYTES = 100 * 1024 * 1024  # 100 MB limit for POST bodies (allow large file uploads)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Dashboard support
        self._web_dist: Optional[Any] = None
    ...
    def _create_agent(
        ...
        # Store selected_skills in session metadata if provided
        if selected_skills is not None and session_id:
            ...
            if db:
                try:
                    ...
                    if session_info:
                        ...
                        # Create new session with selected_skills
                        db._execute_write(lambda conn: conn.execute(
                            """INSERT OR IGNORE INTO sessions (id, source, started_at, selected_skills)
                               VALUES (?, ?, ?, ?)""",
                            (session_id, "api_server", time.time(), json.dumps(selected_skills))
                        ))
                except Exception as e:
                    logger.warning(f"Failed to store selected_skills for session {session_id}: {e}")
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        selected_skills = body.get("selected_skills")  # Optional: list of skill names

        # Validate selected_skills if provided
        if selected_skills is not None:
            if not isinstance(selected_skills, list):
                return web.json_response(
                    {"error": {"message": "'selected_skills' must be an array", "type": "invalid_request_error"}},
                    status=400,
                )
                ...
                if skills_dir.exists():
                    for skill_path in skills_dir.iterdir():
                        if skill_path.is_dir() and not skill_path.name.startswith("."):
                            installed_skills.add(skill_path.name)

                unknown_skills = [s for s in selected_skills if s not in installed_skills]
                if unknown_skills:
                    return web.json_response(
                        {"error": {
                            "message": f"Unknown skills: {', '.join(unknown_skills)}",
        ...
        if provided_session_id:
            ...
            try:
                ...
                if db is not None:
                    ...
                    # Check if session has existing selected_skills
                    if selected_skills is not None:
                        session_info = db.get_session(session_id)
                        if session_info and session_info.get("selected_skills"):
                            existing_skills = json.loads(session_info["selected_skills"])
                            if sorted(existing_skills) != sorted(selected_skills):
                                return web.json_response(
                                    {"error": {
                                        "message": "Cannot change skills mid-session. Start a new session to use different skills.",
                                        "type": "invalid_request_error"
                                    }},
                                    status=400,
                                )
    ...
    def _setup_dashboard_api_routes(self) -> None:
        """Setup Dashboard API routes if GATEWAY_ENABLE_DASHBOARD is set."""
        if not os.getenv("GATEWAY_ENABLE_DASHBOARD"):
            return

        import secrets
        from pathlib import Path

        # Generate session token for Dashboard auth
        self._session_token = secrets.token_urlsafe(32)

        # Resolve web_dist path
        self._web_dist = self._resolve_web_dist_path()

        # Import and instantiate all Dashboard API handlers
        from gateway.platforms.api_server_status import StatusAPIHandlers
        from gateway.platforms.api_server_config import ConfigAPIHandlers
        from gateway.platforms.api_server_env import EnvAPIHandlers
        from gateway.platforms.api_server_sessions import SessionsAPIHandlers
        from gateway.platforms.api_server_logs import LogsAPIHandlers
        from gateway.platforms.api_server_cron import CronAPIHandlers
        from gateway.platforms.api_server_skills import SkillsAPIHandlers
        from gateway.platforms.api_server_tools import ToolsAPIHandlers
        from gateway.platforms.api_server_analytics import AnalyticsAPIHandlers
        from gateway.platforms.api_server_oauth import OAuthAPIHandlers
        from gateway.platforms.api_server_plugins import PluginsAPIHandlers
        from gateway.platforms.api_server_themes import ThemesAPIHandlers
        ...
        env_h = EnvAPIHandlers(self._session_token)
        sessions_h = SessionsAPIHandlers(self._session_token)
        logs_h = LogsAPIHandlers(self._session_token)
        cron_h = CronAPIHandlers(self._session_token)
        skills_h = SkillsAPIHandlers(self._session_token)
        tools_h = ToolsAPIHandlers(self._session_token)
        analytics_h = AnalyticsAPIHandlers(self._session_token)
        oauth_h = OAuthAPIHandlers(self._session_token)
        plugins_h = PluginsAPIHandlers(self._session_token)
        themes_h = ThemesAPIHandlers(self._session_token)
        ...
        # Status API
        self._app.router.add_get("/api/status", status_h.handle_get_status)

        # Config API
        self._app.router.add_get("/api/config", config_h.handle_get_config)
        self._app.router.add_get("/api/config/defaults", config_h.handle_get_config_defaults)
        self._app.router.add_get("/api/config/schema", config_h.handle_get_config_schema)
        self._app.router.add_put("/api/config", config_h.handle_put_config)
        ...
        self._app.router.add_put("/api/env", env_h.handle_set_env)
        self._app.router.add_delete("/api/env", env_h.handle_delete_env)
        self._app.router.add_post("/api/env/reveal", env_h.handle_reveal_env)

        # Sessions API (Dashboard - legacy, keep for compatibility)
        self._app.router.add_get("/api/sessions", sessions_h.handle_get_sessions)
        self._app.router.add_get("/api/sessions/list", sessions_h.handle_get_sessions)
        self._app.router.add_get("/api/sessions/search", sessions_h.handle_search_sessions)
        self._app.router.add_get("/api/sessions/{session_id}", sessions_h.handle_get_session)
        ...
        # Cron API
        self._app.router.add_get("/api/cron/jobs", cron_h.handle_list_jobs)
        self._app.router.add_get("/api/cron/jobs/{job_id}", cron_h.handle_get_job)
        self._app.router.add_post("/api/cron/jobs", cron_h.handle_create_job)
        self._app.router.add_put("/api/cron/jobs/{job_id}", cron_h.handle_update_job)
        self._app.router.add_post("/api/cron/jobs/{job_id}/pause", cron_h.handle_pause_job)
        self._app.router.add_post("/api/cron/jobs/{job_id}/resume", cron_h.handle_resume_job)
        self._app.router.add_post("/api/cron/jobs/{job_id}/trigger", cron_h.handle_trigger_job)
        self._app.router.add_delete("/api/cron/jobs/{job_id}", cron_h.handle_delete_job)

        # Browser API (Chrome CDP)
        self._app.router.add_get("/api/browser/detect-chrome", config_h.handle_detect_chrome)
        self._app.router.add_post("/api/browser/launch-chrome", config_h.handle_launch_chrome)
        self._app.router.add_post("/api/browser/restart-chrome", config_h.handle_restart_chrome)

        # Skills API
        self._app.router.add_get("/api/skills", skills_h.handle_list_skills)
        self._app.router.add_put("/api/skills/toggle", skills_h.handle_toggle_skill)
        self._app.router.add_delete("/api/skills/{skill_name}", skills_h.handle_delete_skill)
        self._app.router.add_get("/api/skills/sources", skills_h.handle_list_sources)
        self._app.router.add_get("/api/skills/search", skills_h.handle_search_skills)
        self._app.router.add_post("/api/skills/upload", skills_h.handle_upload_skill_zip)
        self._app.router.add_post("/api/skills/install", skills_h.handle_install_skill_online)
        self._app.router.add_get("/api/skills/install/tasks", skills_h.handle_list_install_tasks)
        self._app.router.add_get("/api/skills/install/{task_id}", skills_h.handle_get_install_status)
        self._app.router.add_post("/api/skills/install/{task_id}/cancel", skills_h.handle_cancel_install)
        self._app.router.add_post("/api/skills/open-directory", skills_h.handle_open_directory)
        self._app.router.add_post("/api/skills/update-description", skills_h.handle_update_description)

        # Tools API
        self._app.router.add_get("/api/tools/toolsets", tools_h.handle_get_toolsets)

        # Analytics API
        self._app.router.add_get("/api/analytics/usage", analytics_h.handle_get_usage)

        # OAuth API
        self._app.router.add_get("/api/providers/oauth", oauth_h.handle_list_providers)
        self._app.router.add_delete("/api/providers/oauth/{provider_id}", oauth_h.handle_delete_provider)
        self._app.router.add_post("/api/providers/oauth/{provider_id}/start", oauth_h.handle_start_oauth)
        self._app.router.add_post("/api/providers/oauth/{provider_id}/submit", oauth_h.handle_submit_oauth)
        self._app.router.add_get("/api/providers/oauth/{provider_id}/poll/{session_id}", oauth_h.handle_poll_oauth)
        self._app.router.add_delete("/api/providers/oauth/sessions/{session_id}", oauth_h.handle_delete_session)

        # Plugins API
        self._app.router.add_get("/api/dashboard/plugins", plugins_h.handle_get_plugins)
        self._app.router.add_get("/api/dashboard/plugins/rescan", plugins_h.handle_rescan_plugins)
        self._app.router.add_get("/dashboard-plugins/{plugin_name}/{asset_path:.*}", plugins_h.handle_plugin_asset)

        # Themes API
        self._app.router.add_get("/api/dashboard/themes", themes_h.handle_get_themes)
        self._app.router.add_put("/api/dashboard/theme", themes_h.handle_set_theme)

        # Chat API (Unified Chat Interface)
        self._app.router.add_post("/api/chat/sessions/create", chat_h.handle_create_session)
        self._app.router.add_get("/api/chat/sessions", chat_h.handle_list_sessions)
        self._app.router.add_put("/api/sessions/{session_id}", chat_h.handle_update_session)
        ...
        self._app.router.add_get("/api/attachments/{session_id}/{filename}", attachments_h.handle_download_attachment)
        ...
        self._app.router.add_delete("/api/departments/{id}", org_h.handle_delete_department)
        ...
        self._app.router.add_delete("/api/positions/{id}", org_h.handle_delete_position)
        ...
        self._app.router.add_delete("/api/agents/{id}", org_h.handle_delete_agent)
        ...
        self._app.router.add_get(
            ...
            org_h.handle_get_workflow,
        )
        self._app.router.add_post(
            "/api/org/companies/{companyId}/workflow/generate",
            org_h.handle_generate_workflow,
        )
        self._app.router.add_post(
            "/api/org/workflows",
            org_h.handle_create_workflow,
        )
        self._app.router.add_put(
            "/api/org/workflows/{id}",
            org_h.handle_update_workflow,
        )
        self._app.router.add_delete(
            "/api/org/workflows/{id}",
        ...
        if self._web_dist and self._web_dist.exists():
            ...
            self._app.router.add_static('/assets', self._web_dist / 'assets', name='assets')
            self._app.router.add_static('/fonts', self._web_dist / 'fonts', name='fonts')
            self._app.router.add_get('/favicon.ico', self._handle_favicon)
            self._app.router.add_get('/', self._handle_spa_fallback)
            self._app.router.add_get('/{path:.*}', self._handle_spa_fallback)
            logger.info("[Api_Server] Dashboard enabled on http://%s:%d/", self._host, self._port)

    def _resolve_web_dist_path(self) -> Optional[Any]:
        """Resolve web_dist directory path."""
        from pathlib import Path
        import os

        # In Electron mode, use electron-app/dist/renderer
        if os.getenv("HERMES_ELECTRON_MODE") == "true":
            # Try to find electron-app/dist/renderer relative to gateway
            try:
                import gateway
                gateway_path = Path(gateway.__file__).parent.parent
                electron_dist = gateway_path / "electron-app" / "dist" / "renderer"
                if electron_dist.exists():
                    logger.info("[Api_Server] Found Electron renderer at %s", electron_dist)
                    return electron_dist
            except Exception as e:
                logger.warning("[Api_Server] Failed to locate Electron renderer: %s", e)

        # Try to import hermes_cli and look for web_dist
        try:
            import hermes_cli
            cli_path = Path(hermes_cli.__file__).parent
            web_dist = cli_path / "web_dist"
            if web_dist.exists():
                logger.info("[Api_Server] Found web_dist at %s", web_dist)
                return web_dist
        except ImportError:
            pass
    ...
    async def _handle_favicon(self, request: "web.Request") -> "web.Response":
        """Serve favicon.ico."""
        from aiohttp import web
        if self._web_dist and (self._web_dist / "favicon.ico").exists():
            return web.FileResponse(self._web_dist / "favicon.ico")
        return web.Response(status=404)

    async def _handle_spa_fallback(self, request: "web.Request") -> "web.Response":
        """Serve index.html with session token injection for SPA routing."""
        from aiohttp import web

        if not self._web_dist or not (self._web_dist / "index.html").exists():
            return web.json_response({"detail": "Dashboard not available"}, status=404)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Setup Dashboard API routes if enabled
            self._setup_dashboard_api_routes()
