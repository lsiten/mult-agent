...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal ($HERMES_HOME/auth.json active provider)
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate $HERMES_HOME/auth.json for an active Nous provider.
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    old OPENAI_BASE_URL lingers in $HERMES_HOME/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from $HERMES_HOME/.env",
...
def resolve_provider_client(
    ...
    # ── API-key providers (volcengine, zai, kimi, minimax, xiaomi, deepseek, etc.) ───
    # Try to resolve as an API-key provider through PROVIDER_REGISTRY
    try:
        from hermes_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
        ...
        if pconfig and pconfig.auth_type == "api_key":
            try:
                ...
                if api_key and base_url:
                    ...
                    # Use the bare model name (without provider prefix)
                    final_model = model.split("/", 1)[-1] if model and "/" in model else model
                    final_model = _normalize_resolved_model(final_model or "default", provider)
                    wrapped_client = _wrap_if_needed(client, final_model, base_url)
                    return (_to_async_client(wrapped_client, final_model) if async_mode
                            else (wrapped_client, final_model))
            except Exception as exc:
                logger.warning(
                    "resolve_provider_client: provider %r requested but credentials unavailable: %s",
                    provider, exc
                )
                return None, None
    except ImportError:
