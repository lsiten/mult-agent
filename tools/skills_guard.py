...
class ScanResult:
    ...
    def is_safe(self) -> bool:
        """Return True if verdict is 'safe', False otherwise."""
        return self.verdict == "safe"

    @property
    def threats_detected(self) -> int:
        """Return count of critical and high severity findings."""
        return sum(1 for f in self.findings if f.severity in ("critical", "high"))

    @property
    def threat_details(self) -> List[str]:
        """Return list of threat descriptions for critical/high findings."""
        return [
            f"{f.pattern_id}: {f.description} (line {f.line})"
    ...
    def threats(self) -> List[str]:
        """Return list of threat pattern IDs for critical/high findings."""
        return [
            f.pattern_id
            for f in self.findings
            if f.severity in ("critical", "high")
...
def scan_skill(skill_path: Path, source: str = "community") -> ScanResult:
    ...
    if skill_path.is_dir():
        ...
        # Pattern scanning on each file (skip documentation)
        SKIP_EXTENSIONS = {'.md', '.txt', '.rst', '.pdf', '.doc', '.docx'}
        for f in skill_path.rglob("*"):
            if f.is_file():
                # Skip documentation files - they often contain example paths
