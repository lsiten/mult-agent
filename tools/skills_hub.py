...
class SkillBundle:
    ...
    @classmethod
    def from_zip(cls, zip_path: Path, source: str = "upload") -> "SkillBundle":
        """
        Create SkillBundle from uploaded ZIP file.
        ...
        Returns:
            SkillBundle ready for validation and installation
        ...
        logger.info(f"[from_zip] Checking ZIP file: {zip_path}")
        logger.info(f"[from_zip] File size: {zip_path.stat().st_size} bytes")
        ...
        if not zipfile.is_zipfile(zip_path):
            ...
            try:
                with open(zip_path, 'rb') as f:
                    header = f.read(4)
                    logger.error(f"[from_zip] Not a valid ZIP file. First 4 bytes: {header.hex()}")
            except Exception as e:
                logger.error(f"[from_zip] Cannot read file: {e}")
        ...
        with zipfile.ZipFile(zip_path, 'r') as zf:
            # Log ZIP contents for diagnosis
            all_files = zf.namelist()
            logger.info(f"[from_zip] ZIP contains {len(all_files)} files: {all_files[:10]}")  # First 10
            ...
            if compression_ratio > 4.0:
                raise ValueError(
                    f"Suspicious ZIP file: compression ratio {compression_ratio:.1f}:1 exceeds limit (4:1). "
                    "This may be a ZIP bomb attack."
                )

            if uncompressed_size > 200 * 1024 * 1024:  # 200MB
                raise ValueError(
                    f"ZIP file too large when extracted: {uncompressed_size / 1024 / 1024:.1f}MB "
                    "(max 200MB)"
                )
            ...
            for info in zf.infolist():
                ...
                try:
                    ...
                    raise ValueError(f"Unsafe file path in ZIP: {rel_path} - {e}")

                # Check for symlinks or absolute paths
                if info.external_attr & 0x20000000:  # Symlink flag
                    raise ValueError(f"Symlinks not allowed in skill packages: {rel_path}")
                ...
                try:
                    ...
                    logger.warning(f"Failed to read file {rel_path} from ZIP: {e}")
                    continue

            # Find and parse SKILL.md or skill.yaml
            skill_meta_file = None
            has_skill_md = False
            has_skill_yaml = False

            for path in files.keys():
                if path.endswith("/SKILL.md") or path == "SKILL.md":
                    skill_meta_file = path
                    has_skill_md = True
                    break
                elif path.endswith("/skill.yaml") or path == "skill.yaml":
                    skill_meta_file = path
                    has_skill_yaml = True
                    break

            if not skill_meta_file:
                raise ValueError(
                    "Invalid skill package: missing SKILL.md or skill.yaml in root directory"
                )

            # Parse metadata
            content = files[skill_meta_file]
            if isinstance(content, bytes):
                content = content.decode('utf-8')

            if skill_meta_file.endswith(".md"):
                # Parse YAML frontmatter
                metadata = cls._parse_frontmatter(content)

                # If SKILL.md has no frontmatter, create default metadata and a skill.yaml
                if not metadata:
                    logger.info("SKILL.md has no frontmatter, auto-generating skill.yaml")
                    # Derive skill name from ZIP file name or use default
                    derived_name = zip_path.stem.replace("-", "_").replace(" ", "_")
                    metadata = {
                        "name": derived_name,
                        "description": "Skill imported from SKILL.md"
                    }
                    # Create skill.yaml in files
                    import yaml
                    skill_yaml_content = yaml.dump(metadata, default_flow_style=False)
                    files["skill.yaml"] = skill_yaml_content
                    logger.info(f"Created skill.yaml with name: {derived_name}")
            else:
                # Parse YAML file
                import yaml
            ...
            if not isinstance(metadata, dict):
                raise ValueError("Invalid skill metadata: expected YAML dict")

            # Extract skill name
            skill_name = metadata.get("name")
            if not skill_name:
                raise ValueError("Invalid skill metadata: missing 'name' field")

            # Validate skill name
            try:
                skill_name = _validate_skill_name(skill_name)
            except ValueError as e:
                raise ValueError(f"Invalid skill name: {e}")
        ...
        return cls(
            ...
            trust_level="community",  # Uploaded skills default to community trust
            metadata=metadata
        )
...
def _normalize_skill_structure(skill_dir: Path, bundle: SkillBundle) -> None:
    """
    Normalize skill structure after installation.

    Ensures skill.yaml exists by extracting metadata from SKILL.md or DESCRIPTION.md.
    This allows consistent API access to skill metadata without parsing markdown.
    """
    import yaml
    ...
    # If skill.yaml already exists, nothing to do
        ...
        try:
            ...
            if content.startswith("---"):
                ...
                if len(parts) >= 3:
                    ...
                    logger.info(f"Created skill.yaml for {skill_dir.name} from SKILL.md")
        except Exception as e:
            logger.warning(f"Failed to extract from SKILL.md for {skill_dir.name}: {e}")
    ...
    # Try DESCRIPTION.md (category directories)
        ...
        try:
            ...
            logger.info(f"Created skill.yaml for {skill_dir.name} from DESCRIPTION.md")
        except Exception as e:
            logger.warning(f"Failed to extract from DESCRIPTION.md for {skill_dir.name}: {e}")
...
class OptionalSkillSource(SkillSource):
    ...
    $HERMES_HOME/skills/ during setup.  They are discoverable via the Skills Hub
...
def install_from_quarantine(
    ...
    # Normalize skill structure: ensure skill.yaml exists
    _normalize_skill_structure(install_dir, bundle)
