    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                # v7: add selected_skills column to sessions table for skill selection feature
                # and metadata column to messages table for tool_use/skill_use types
                try:
                    cursor.execute('ALTER TABLE sessions ADD COLUMN "selected_skills" TEXT')
                except sqlite3.OperationalError:
                    pass  # Column already exists
                try:
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   reasoning, reasoning_details, codex_reasoning_items, attachments, metadata)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
        ...
        for row in rows:
            ...
            if msg.get("metadata"):
                try:
                    ...
                    logger.warning("Failed to deserialize metadata in get_messages, falling back to None")
                    msg["metadata"] = None
            result.append(msg)
        return result
