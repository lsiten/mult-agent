...
async def auth_middleware(request: Request, call_next):
    ...
    # Disable auth in Electron mode (single-user desktop app)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``$HERMES_HOME/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
#          → persists to $HERMES_HOME/.anthropic_oauth.json AND credential pool
...
def _discover_user_themes() -> list:
    """Scan $HERMES_HOME/dashboard-themes/*.yaml for user-created themes."""
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    $HERMES_HOME/plugins/<name>/dashboard/manifest.json
...
# In Electron mode, don't mount the SPA - only serve APIs
# The Electron app loads the frontend directly from its bundled files
...
def start_server(
    ...
    # Don't auto-open browser in Electron mode
    if open_browser and not _is_electron_mode():
        import threading
        import webbrowser
