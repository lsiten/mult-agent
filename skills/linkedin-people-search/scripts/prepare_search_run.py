...
def build_record_context(job_record, weight_record):
    ...
    keyword_query = " ".join(unique_keep_order([title, *skills[:4], *keywords[:2]]))
    ...
    return {
        ...
        "manual_filters": {
            ...
            "Current title": category or title,
...
def main():
    parser = argparse.ArgumentParser(description="Prepare LinkedIn people search context from job and weight JSON.")
    ...
    parser.add_argument("--output-root", help="Output root directory. Defaults to <job_dir>/linkedin.")
    ...
    output_root = Path(args.output_root).expanduser().resolve() if args.output_root else (job_file.parent / "linkedin").resolve()
    slug = safe_name(args.slug or job_file.stem)
    ...
    search_context = {
        "schema_version": "1.0",
        "task": "linkedin_people_search_context",
        "generated_at": now_iso(),
        "job_file": str(job_file),
        "weights_file": str(weights_file),
        "search_url": SEARCH_URL,
