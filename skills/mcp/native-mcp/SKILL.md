...
Add MCP servers to `$HERMES_HOME/config.yaml` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `$HERMES_HOME/config.yaml`
...
No `mcp_servers` key in `$HERMES_HOME/config.yaml`, or it's empty. Add at least one server.
