...
**Electron mode**: The desktop app sets `HERMES_HOME` to `~/Library/Application Support/hermes-agent-electron/` and `HERMES_ELECTRON_MODE=true`. Always use `get_hermes_home()` to respect these settings. Never hardcode fallback paths like `Path.home() / ".hermes"` — they bypass Electron's configuration.
...
| HOME / `$HERMES_HOME/` | Your real config+auth.json | Temp dir per test |
